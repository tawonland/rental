    ...
    */
    private function get_filtering()
    {
      ...
      for($i = 0; $i < count($mColArray); $i++)
      {
        ...
        {
          $sWhere1 .= $this->select[$this->columns[$i]] . " LIKE '%" . $mColArray[$i]['search']['value'] . "%' AND ";
        }
        if($sSearch != '')
        {
          $sWhere2 .= $this->select[$this->columns[$i]] . " LIKE '%" . $sSearch . "%' OR ";              
        }
      }

      $sWhere1 = substr_replace($sWhere1, '', -4);
